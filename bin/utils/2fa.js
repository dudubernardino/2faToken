// Reference: https://github.com/multiOTP/genOTP
...
function hotp(options) {
  ...
  return value
    .toString()
    .padStart(options.codeLength, "0")
    .substring(0, options.codeLength);
}
...
OTP.prototype = {
  ...
  generate: function (options) {
    ...
    if ("motp" == this.type) {
      let epochTimeTen = Math.floor(
        (Date.now() / 1000 - this.bias) / this.period
      );
      for (let i = 0; i < this.values; i++) {
        _result.push(
          md5(
            String(epochTimeTen + i) +
              String(this.secret) +
              String(this.pincode)
          ).substring(0, this.digits)
        );
      }
    } else {
      if ("totp" == this.type) {
...
export const get2fa = (secret) => {
  const currentUnixTime = Math.floor(Date.now() / 1000);
  const counter = Math.floor(currentUnixTime / 30);
  ...
  const validityPeriod = 30 - (currentUnixTime % 30);
